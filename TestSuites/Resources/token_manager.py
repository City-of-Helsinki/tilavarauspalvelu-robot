...
def force_refresh_access_token():
    ...
    logger.info(
        f"- ACCESS_TOKEN: {'Present' if access_token else 'Missing'} (length: {len(access_token) if access_token else 0})"
    )
    logger.info(
        f"- REFRESH_TOKEN: {'Present' if refresh_token else 'Missing'} (length: {len(refresh_token) if refresh_token else 0})"
    )
