...
def main():
    ...
    # Overall statistics
    success_percentage = (
        (len(combined_stats["2xx_success"]) / total_requests_all * 100) if total_requests_all > 0 else 0
    )
    error_percentage = (
        ((len(combined_stats["4xx_client_error"]) + len(combined_stats["5xx_server_error"])) / total_requests_all * 100)
    ...
    print(
        f"❌ Error Rate (4xx+5xx): {len(combined_stats['4xx_client_error']) + len(combined_stats['5xx_server_error'])} ({error_percentage:.1f}%)"
    )
